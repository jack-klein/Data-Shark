...
my %sys_list = (
 'sDIO' => {
  'get_dios' => {
      'cd_type'       => 'DBI',
      'cd_stmt'       => 'select cd_id, cd_namespace, cd_name, cd_version, cd_sysclass, cd_type, cd_return, cd_cache, cd_stmt, cd_stmt_noarg, cd_profile, cd_repl, cd_action, cd_audit from core_dio order by cd_namespace, cd_name, cd_version, cd_db_hook',
      'out_keys'   => { 'cd_id' => {pos => 0}, 'cd_namespace' => {pos => 1}, 'cd_name' => {pos => 2}, 'cd_version' => {pos => 3}, 'cd_sysclass' => {pos => 4}, 'cd_type' => {pos => 5}, 'cd_return' => {pos => 6}, 'cd_cache' => {pos => 7}, 'cd_stmt' => {pos => 8}, 'cd_stmt_noarg' => {pos => 9}, 'cd_profile' => {'pos' => 10}, 'cd_repl' => {'pos' => 11}, 'cd_action' => {'pos' => 12}, 'cd_audit' => {'pos' => 13}, 'cd_db_hook' => {'pos' => 14} },
...
sub register_dio {
  ...
  $_DIOS->{ $args{'tag'} } = $args{'func'};
}

sub call_dio {
  my ($tag, $args) = @_;

  return $_DIOS->{$tag}->($args);
}
...
sub _init {
  ...
  foreach my $dions (sort {$a cmp $b} keys %{ $dio_list }) {
    ...
    for ($dioh->{'cd_repl'}) {
      /^dispatch$/ && do {
        $stmt .= "Data::Shark::DIO::register_dio( tag => q{${ns}::${dions}::${diotag}},\n";
        $stmt .= "                                func => sub {\n";
        last;
      };
      /^method$/ && do {
...
#        }
    ...
    $stmt .= '  my $dbi = ' . ($dioh->{'cd_db_hook'} || $config->{'dbi_func'}) . ';' . "\n"; 
    $stmt .= '  my $sth;' . "\n\n";
        ...
        if ($in_keys) {
          ...
          foreach my $k (@{ $in_keys }{ keys %{ $in_keys } }) {
            for ($k->{'name'}) {
              ...
              /^mysql_limit$/ && do { $extra .= ' . q{ limit } . $args->{\'mysql_limit\'} '; last; };
              /^offset$/ && do { $extra .= ' offset \' . $args->{\'offset\'} . \''; last; };
            }
          }
          $stmt .= '  $sth = $dbi->db_prep($sql' . ($extra ? $extra : '') . ');' . "\n";
          if ($have_tags) {
            $stmt .= '  my @binds;' . "\n";
            foreach my $k (@{ $in_keys }{ sort {$a <=> $b} keys %{ $in_keys } }) {
              next if exists $k->{'no_bind'} || ($k->{'name'} =~ /(mysql_limit|limit|offset)/);
              if ($k->{'opt'} eq 'tag_based') {
          ...
          #$dioh->{'cd_stmt'} =~ s/'/\\'/g;
          $stmt .= '  $sth = $dbi->db_sql(q{' . $dioh->{'cd_stmt'} . '});' . "\n\n";
        }
        last;
      };
      /^SQL::Abstract$/ && do {
